...
def f_core(t): return math.exp(-0.5*(t/s0)**2)
def f_tail(t): return (t*t + chia*chia)**-1.5      # ~ t^-3 at large t
def f(t):      return (1-wtail)*f_core(t)/(s0*math.sqrt(2*math.pi)) + wtail*f_tail(t)
...
print(f"   -> {'PASS' if qual else 'FAIL'}: 4th moment is quadratic, not log; "
...
print("\nNOTE: exact eps_M(+5.1..+17.1%), k_opt=1.84, eta_max=1.197 require the "
      ...
      "the divergence LAWS underpinning them. The eps_M magnitudes remain "
      "[UNVERIFIED-CODE] pending the author's quadrature or Geant4 output.")
