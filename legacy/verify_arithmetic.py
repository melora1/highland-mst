...
def line(name, got, want, unit="", tol=0.02):
    ...
    print(
        f"[{'PASS' if ok else 'FAIL'}] {name:38s} got={got:.4g}{unit:>6s}  paper={want:g}{unit}"
    )
    return ok
...
for p, want in [(1.0, 90), (2.0, 45), (3.5, 26), (6.0, 15)]:
    ok &= line(
        f"delta @ {p} GeV/c", 0.3 * 1.0 * 0.30 / p * 1e3, want, " mrad", tol=0.03
    )
...
for p, want in [(1.0, 93), (2.0, 47), (3.5, 27), (6.0, 15)]:
    ok &= line(
        f"thetacut_opt @ {p}", 1.84 * theta0(p, xX0) * 1e3, want, " mrad", tol=0.04
    )
for p, e200, gain in [
    (1.0, 0.946, 27),
    (2.0, 0.693, 73),
    ...
    ok &= line(
        f"SNR gain @ {p} (table self-consistency)",
        (1.197 / e200 - 1) * 100,
        gain,
        " %",
        tol=0.03,
...
def m4_tail(cut, a=1e-3):  # projected 1D tail f ~ theta^-3 => (theta^2+a^2)^-1.5
    ...
    for i in range(1, n + 1):
        t = i * h
        s += t**4 * (t**2 + a**2) ** -1.5  # <theta^4>: integrand theta^4 * f
    return s * h
