...
def _run_tag_from_figdir(figdir):
    ...
    safe = "".join(c if (c.isalnum() or c in "-_") else "_" for c in run_dir)
...
def _load(outdir):
    ...
    artifact = pd.read_csv(os.path.join(outdir, "artifact.csv"), index_col=0).squeeze(
        "columns"
    )
...
def plot_slices(imgs, figdir, min_count=20):
    ...
    for i, (ax, name) in enumerate(zip(np.atleast_1d(axes), names)):
        ...
        ax.set_title(
            name.replace("_", r"$_{\mathrm{") + "}}$" if "_" in name else name,
            fontsize=9,
        )
...
def plot_metrics_bars(metrics, figdir):
    ...
    for i, (ax, (col, err, ylab)) in enumerate(zip(axes, panels)):
        ...
        ax.bar(
            xpos, metrics[col], yerr=metrics[err], capsize=3, color=colors, linewidth=0
        )
        ax.set_ylabel(ylab)
        # LaTeX tick labels
        labels = [
            ...
            if t == "I_p"
            else r"$I_{\mathrm{ideal}}$"
            if t == "I_ideal"
            else r"$I_{\mathrm{const}}$"
            if t == "I_const"
            else r"$I_{Q}$"
            if t == "I_Q"
            else r"$I_{Q,\mathrm{adap}}$"
...
def plot_artifact_map(imgs, figdir):
    ...
    ax.set_xlabel(r"$x$ (cm)")
    ax.set_ylabel(r"$y$ (cm)")
    fig.colorbar(im, ax=ax, label=r"$I_{\mathrm{nom}}-I_{Q}$", shrink=0.85)
...
def plot_artifact_summary(artifact, figdir):
    ...
    fig.subplots_adjust(left=0.10, right=0.97, top=0.88, bottom=0.22, wspace=0.45)
    ...
    bars = ax.bar(labels_a, vals_a, color=["#D55E00", "#009E73"], linewidth=0)
    ...
    # (b) fractions
    ax = axes[1]
    vals_b = [artifact["orth_fraction"] * 100, artifact["reduction"] * 100]
    labels_b = ["non-rescaling\northogonal fraction", "per-momentum\nreduction"]
    bars = ax.bar(labels_b, vals_b, color=["#D55E00", "#009E73"], linewidth=0)
...
def plot_psf_profile(
    imgs, figdir, image_name="I_nom", z_band=5.0, oversample=1, min_bin_count=5
):
    ...
    latex_name = (
        r"$I_{\mathrm{" + image_name.replace("I_", "") + r"}}$"
        if image_name.startswith("I_")
        else image_name
    )
    print(
