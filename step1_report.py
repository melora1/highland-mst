...
def _X(t):
    return (MATERIALS["Al"]["rho"] * t[0], MATERIALS["Cu"]["rho"] * t[1],
            MATERIALS["Pb"]["rho"] * t[2])


# ------------------------------------------------------------------ 0 closure
print(RULE); print("0.  CLOSURE"); print(RULE)
...
for p in (1.0, 6.0):
    ...
    print(f"    p={p:4.1f}  eps_M[p(X), no loss]={100*r['eps_M']:.9f}%  "
          f"eps_M[Eq.(A14)]={100*r['eps_M_0']:.9f}%  "
          f"diff={100*(r['eps_M']-r['eps_M_0']):+.2e} pp")
el.energy_after = _real

print("\n0c. constant-p baseline vs repo eps_quadrature, and vs published "
      "Table I:")
print(f"    {'p':>5} {'calibrate':>11} {'eps_quad':>11} {'Table I':>9} "
      f"{'code-paper (pp)':>16}")
Xax = _X(AXIAL)
for p in MOMENTA:
    mine = 100 * el.calibrate(*AXIAL, p)["eps_M_0"]
    repo = 100 * float(eps_M_constp(p, *Xax)[0])
    print(f"    {p:5.1f} {mine:11.4f} {repo:11.4f} {PUBLISHED[p]:9.3f} "
...
# ------------------------------------------------------------------ 1 profile
print(); print(RULE)
print("1.  MOMENTUM PROFILE AND ADAPTIVE SLICING  [1.1, 1.2]"); print(RULE)
print(f"axial path: Al {AXIAL[0]} cm + Cu {AXIAL[1]} cm, "
...
for p in MOMENTA:
    r = el.calibrate(*AXIAL, p)
    rows.append(r)
    print(f"{p:5.1f} {r['p_out']:8.4f} {100*r['dp_over_p']:+8.2f}% "
...
# ------------------------------------------------------------------ 2 converge
print(); print(RULE)
print("2.  SLICING CONVERGENCE"); print(RULE)
...
for tol in (0.02, 0.01, 0.005, 0.002, 0.001):
    ...
    print(f"{tol:7.3f} {r['n_slices']:8d} {100*r['eps_M']:16.6f} "
...
print(RULE)
print(f"{'p_in':>5} {'Dp/p %':>8} {'k const-p':>10} {'k p(X)':>8} "
      f"{'F_c':>9} {'eps const-p %':>14} {'eps p(X) %':>12} {'shift pp':>10}")
for p, r in zip(MOMENTA, rows):
    print(f"{p:5.1f} {100*r['dp_over_p']:+8.2f} {r['k_0']:10.3f} "
...
# ------------------------------------------------------------------ 4 deployed
print(); print(RULE)
print("4.  WHAT THE DEPLOYED ESTIMATOR SEES"); print(RULE)
...
for p, r in zip(MOMENTA, rows):
    print(f"{p:5.1f} {100*r['eps_M_0']:14.3f} {100*r['eps_M']:12.3f} "
...
print(f"\n{'f':>6} {'p_out(1 GeV)':>13} {'eps p(X) % @1':>15} "
      f"{'eps p(X) % @6':>15}")
...
for f in (0.95, 1.00, 1.05, 1.10):
    ...
    print(f"{f:6.2f} {a['p_out']:13.4f} {100*a['eps_M']:15.4f} "
          f"{100*b['eps_M']:15.4f}")
...
# ------------------------------------------------------------------ 6 off-Cu
print(); print(RULE)
print("6.  OFF-Cu PATH (25 cm Al)  -- input to Step 3.4"); print(RULE)
print(f"x/X0 = {el.x_over_X0(el.ordered_path(*OFFCU)):.4f}, "
...
print(RULE)
print(f"{'p':>5} {'theta0 single-log':>19} {'theta0 per-slice':>18} "
      f"{'ratio':>8}")
for p in MOMENTA:
    path = el.ordered_path(*AXIAL)
    sl, _ = el.slice_path(path, p)
    th_ok, _ = el.highland_pofx(sl, el.x_over_X0(path))
    q = sum(float(theta0_highland(s["p"],
                                  s["dx"] / MATERIALS[s["mat"]]["X0"])) ** 2
            for s in sl)
    print(f"{p:5.1f} {th_ok*1e3:19.4f} {math.sqrt(q)*1e3:18.4f} "
...
for p in MOMENTA:
    ...
    print(f"p={p:4.1f}  beta_eff={r['beta_eff']:.6f}  "
