...
All figures are written to a single folder: out/figs
The input directory is still the analysis/theory run directory, but figure
outputs are centralized and prefixed by the source run directory name.
"""
...
def _fig_root(outdir: str | Path) -> tuple[Path, str]:
    src = Path(outdir).resolve()
    if src.name == "out":
        root = src / "figs"
        prefix = "out"
    elif src.parent.name == "out":
        root = src.parent / "figs"
        prefix = src.name
    else:
        root = src / "figs"
        prefix = src.name
...
def _save(fig, outdir, stem):
    root, prefix = _fig_root(outdir)
    pdf = root / f"{prefix}_{stem}.pdf"
    png = root / f"{prefix}_{stem}.png"
    fig.tight_layout()
...
def plot_theory(outdir):
    ...
    eps = [np.sqrt(rp["R"] * rp["B"] * mu2_eta(float(e), rp["B"], 2)) - 1.0 for e in eta]
...
def plot_images(outdir):
    ...
    fig.colorbar(im, ax=axes.ravel().tolist(), label="weight difference", shrink=0.85)
    fig.subplots_adjust(wspace=0.15, right=0.88)

    _save(fig, outdir, "difference_maps")


def plot_gradient(outdir):
    out = Path(outdir)
    z = np.load(out / "gradient_maps.npz")
    ...
    for ax, v, l in zip(axes, vals, labels):
        ...
        ax.set_xlabel("x (cm)")
        ax.set_title(l, fontsize=8.5)
        fig.colorbar(im, ax=ax, shrink=0.76)

    axes[0].set_ylabel("y (cm)")
    fig.subplots_adjust(wspace=0.38)

    _save(fig, outdir, "gradient_causal_maps")
...
def main():
    ...
    if a.kind in ("theory", "all") and (p / "theory_collapse.csv").exists():
        plot_theory(a.outdir)
    if a.kind in ("images", "all") and (p / "images.npz").exists():
        plot_images(a.outdir)
    if a.kind in ("gradient", "all") and (p / "gradient_maps.npz").exists():
        plot_gradient(a.outdir)
