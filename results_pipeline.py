...
if _config.STEER_COMPENSATION != "per_setting":
    print(
        f"[results_pipeline] config.STEER_COMPENSATION was "
...
def _preflight_correlation_check():
    ...
    med = [
        np.median(simulate.simulate_setting(p, n=15000, mode="gauss").poca_x)
        for p in MOMENTA
    ]
    ...
    print(
        f"[results_pipeline] production steering check under "
...
def build_weights(df):
    ...
    xX0_ref = (
        X_al_ref / MATERIALS["Al"]["rho"] / MATERIALS["Al"]["X0"]
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        w_p = np.where(tspace_ref > 0, (dth / ((1.0 + eps_p) * tspace_ref)) ** 2, 0.0)
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        w_ideal = np.where(trms_true > 0, (df.dth_true.values / trms_true) ** 2, 0.0)

    with np.errstate(divide="ignore", invalid="ignore"):
        eps_ref_per_event = np.where(tspace_ref > 0, trms_ref / tspace_ref - 1.0, 0.0)
    eps_bar = float(
        np.average(eps_ref_per_event, weights=np.where(w_nom > 0, w_nom, 0.0))
    )
    with np.errstate(divide="ignore", invalid="ignore"):
        w_const = np.where(
            tspace_ref > 0, (dth / ((1.0 + eps_bar) * tspace_ref)) ** 2, 0.0
        )
...
def speckle_metric(img, counts, min_count=20, n_boot=500, rng=None):
    ...
    for i in range(n_boot):
        ...
        mean = np.mean(sample)
        boot[i] = np.std(sample, ddof=1) / np.abs(mean) if mean != 0 else np.nan
...
def main():
    ...
    print(
        f"Simulating {args.n} muons/setting x {len(MOMENTA)} settings "
        f"(seed={args.seed}) ..."
    )
    frames = []
    for p in MOMENTA:
        df = simulate.simulate_setting(
            p, n=args.n, seed_offset=args.seed, sampler=sampler
        )
        frames.append(df)
    ...
    # ------------------------------------------------------------------ adaptive cut
    theta_cut_opt = optimal_cut(
        cat.p_meas.values, cat.X_al_ref.values, cat.X_cu_ref.values, cat.X_pb_ref.values
    )
    ...
    trms_adapt = theta_RMS_at_cut(
        df_adapt.p_meas.values,
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        w_Q_adapt = np.where(
            trms_adapt > 0, (df_adapt.dth_reco.values / trms_adapt) ** 2, 0.0
        )
    ...
    xX0_adapt = (
        cat.X_al_ref.values / MATERIALS["Al"]["rho"] / MATERIALS["Al"]["X0"]
    ...
    p_out_vals = p_exit(
        p_meas_vals, df.X_al_ref.values, df.X_cu_ref.values, df.X_pb_ref.values
    )
    with np.errstate(divide="ignore", invalid="ignore"):
        dpp = np.where(p_meas_vals > 0, p_out_vals / p_meas_vals - 1.0, 0.0)
    w_pos = np.where(w_nom > 0, w_nom, 0.0)
    print(
        f"event-weighted mean Delta p/p through the reference path    = "
    ...
    for pset in sorted(df.p_set.unique()):
        ...
        wm = w_pos[m]
        mean_dpp = (
            100 * np.average(dpp[m], weights=wm) if wm.sum() > 0 else float("nan")
        )
        print(f"  p_set={pset:4.1f}: mean Delta p/p = {mean_dpp:+.2f}%  (n={m.sum()})")
    ...
    # ------------------------------------------------------------------ adaptive image
    sample_a = np.stack(
        [df_adapt.poca_x.values, df_adapt.poca_y.values, df_adapt.poca_z.values], 1
    )
