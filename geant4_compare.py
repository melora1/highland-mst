...
def load_angles(path):
    ...
    if p.suffix.lower()==".csv":
        d=pd.read_csv(p)
        if "theta_space" not in d.columns: raise ValueError(f"{p}: CSV needs theta_space column")
...
def sample_moments(a, cut):
    keep=a[a<cut]
    if keep.size==0: return dict(Fc=0.0,theta_rms=np.nan,M4=np.nan,n=0)
    return dict(Fc=keep.size/a.size,theta_rms=float(np.sqrt(np.mean(keep**2))),
                M4=float(np.mean(keep**4)),n=int(keep.size))


def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--file",action="append",required=True,help="label=angles.txt (repeat for Urban/Wentzel)")
    ap.add_argument("--path",choices=["AlCu","Al25"],default="AlCu")
    ap.add_argument("--p",type=float,required=True)
    ap.add_argument("--energy-loss",action="store_true",help="compare to the segmented p(X) approximation; otherwise constant-p control")
    ...
    rows=[]
    if a.energy_loss:
        if a.path not in PATH_ORDERED: raise ValueError("energy-loss benchmark requires an ordered path")
    ...
    for k in a.k:
        ...
        for lab,ang in data.items():
            ...
            rows.append(dict(model=lab,k=k,theta_cut=cut,Fc_g4=g["Fc"],Fc_quad=q["Fc"],
                             theta_rms_g4=g["theta_rms"],theta_rms_quad=q["theta_rms"],
                             M4_g4=g["M4"],M4_quad=q["M4"],
                             rms_frac=(g["theta_rms"]/q["theta_rms"]-1.0),
                             Fc_diff=g["Fc"]-q["Fc"],M4_frac=(g["M4"]/q["M4"]-1.0)))
    d=pd.DataFrame(rows)
